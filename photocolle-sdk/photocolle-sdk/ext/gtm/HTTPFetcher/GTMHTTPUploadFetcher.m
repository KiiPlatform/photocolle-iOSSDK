...
@interface DCGTMHTTPFetcher (ProtectedMethods)
...
@interface DCGTMHTTPUploadFetcher ()
+ (DCGTMHTTPUploadFetcher *)uploadFetcherWithRequest:(NSURLRequest *)request
...
- (void)handleResumeIncompleteStatusForChunkFetcher:(DCGTMHTTPFetcher *)chunkFetcher;

- (void)uploadFetcher:(DCGTMHTTPFetcher *)fetcher
...
-(BOOL)chunkFetcher:(DCGTMHTTPFetcher *)chunkFetcher
          willRetry:(BOOL)willRetry
           forError:(NSError *)error;

- (void)chunkFetcher:(DCGTMHTTPFetcher *)chunkFetcher
    finishedWithData:(NSData *)data
               error:(NSError *)error;
@end

@interface DCGTMHTTPUploadFetcher (PrivateMethods)
...
@implementation DCGTMHTTPUploadFetcher
...
+ (DCGTMHTTPUploadFetcher *)uploadFetcherWithRequest:(NSURLRequest *)request
                                    ...
                                    uploadMIMEType:(NSString *)uploadMIMEType
                                         chunkSize:(NSUInteger)chunkSize
                                    fetcherService:(DCGTMHTTPFetcherService *)fetcherService {
  DCGTMHTTPUploadFetcher *fetcher = [self uploadFetcherWithRequest:request
...
+ (DCGTMHTTPUploadFetcher *)uploadFetcherWithLocation:(NSURL *)locationURL
                                   uploadFileHandle:(NSFileHandle *)fileHandle
                                     uploadMIMEType:(NSString *)uploadMIMEType
                                          chunkSize:(NSUInteger)chunkSize
                                     fetcherService:(DCGTMHTTPFetcherService *)fetcherService {
  DCGTMHTTPUploadFetcher *fetcher = [self uploadFetcherWithRequest:nil
                                                  fetcherService:fetcherService];
...
+ (DCGTMHTTPUploadFetcher *)uploadFetcherWithRequest:(NSURLRequest *)request
  ...
  DCGTMHTTPUploadFetcher *fetcher;
  if (fetcherService) {
    fetcher = [fetcherService fetcherWithRequest:request
                                    fetcherClass:self];
  } else {
    fetcher = (DCGTMHTTPUploadFetcher *) [self fetcherWithRequest:request];
  }
  return fetcher;
}
...
- (BOOL)beginFetchWithDelegate:(id)delegate
  ...
  DCGTMAssertSelectorNilOrImplementedWithArgs(delegate, finishedSEL,
        @encode(DCGTMHTTPFetcher *), @encode(NSData *), @encode(NSError *), 0);
  ...
  needsManualProgress_ = ![DCGTMHTTPFetcher doesSupportSentDataCallback];
...
- (void)uploadNextChunkWithOffset:(NSUInteger)offset
  ...
  // make a new fetcher
  //
  DCGTMHTTPFetcher *chunkFetcher;

  chunkFetcher = [DCGTMHTTPFetcher fetcherWithRequest:chunkRequest];
  ...
  if (!didFetch) {
    ...
    NSError *error = [NSError errorWithDomain:kDCGTMHTTPFetcherErrorDomain
                                         code:kDCGTMHTTPFetcherErrorChunkUploadFailed
                                     userInfo:nil];
...
- (void)chunkFetcher:(DCGTMHTTPFetcher *)chunkFetcher finishedWithData:(NSData *)data error:(NSError *)error {
...
-(BOOL)chunkFetcher:(DCGTMHTTPFetcher *)chunkFetcher willRetry:(BOOL)willRetry forError:(NSError *)error {
  if ([error code] == 308
      && [[error domain] isEqual:kDCGTMHTTPFetcherStatusDomain]) {
...
- (void)uploadFetcher:(DCGTMHTTPFetcher *)chunkFetcher
         didSendBytes:(NSInteger)bytesSent
       totalBytesSent:(NSInteger)totalBytesSent
totalBytesExpectedToSend:(NSInteger)totalBytesExpected {
...
- (DCGTMHTTPFetcher *)activeFetcher {
  if (chunkFetcher_) {
