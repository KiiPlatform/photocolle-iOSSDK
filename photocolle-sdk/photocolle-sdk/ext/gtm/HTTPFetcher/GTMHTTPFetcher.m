...
#if DCGTM_BACKGROUND_FETCHING
#import <UIKit/UIKit.h>
#endif

static id <DCGTMCookieStorageProtocol> gDCGTMFetcherStaticCookieStorage = nil;
static Class gDCGTMFetcherConnectionClass = nil;


NSString *const kDCGTMHTTPFetcherStartedNotification           = @"kDCGTMHTTPFetcherStartedNotification";
NSString *const kDCGTMHTTPFetcherStoppedNotification           = @"kDCGTMHTTPFetcherStoppedNotification";
NSString *const kDCGTMHTTPFetcherRetryDelayStartedNotification = @"kDCGTMHTTPFetcherRetryDelayStartedNotification";
NSString *const kDCGTMHTTPFetcherRetryDelayStoppedNotification = @"kDCGTMHTTPFetcherRetryDelayStoppedNotification";

NSString *const kDCGTMHTTPFetcherErrorDomain       = @"com.google.DCGTMHTTPFetcher";
NSString *const kDCGTMHTTPFetcherStatusDomain      = @"com.google.HTTPStatus";
NSString *const kDCGTMHTTPFetcherErrorChallengeKey = @"challenge";
NSString *const kDCGTMHTTPFetcherStatusDataKey     = @"data";  // data returned with a kDCGTMHTTPFetcherStatusDomain error
...
@interface DCGTMHTTPFetcher ()

@property (copy) NSString *temporaryDownloadPath;
@property (retain) id <DCGTMCookieStorageProtocol> cookieStorage;
...
+ (void)initialize {
  ...
  // thread-safe manner
  if (!gDCGTMFetcherStaticCookieStorage) {
    Class cookieStorageClass = NSClassFromString(@"DCGTMCookieStorage");
    if (cookieStorageClass) {
      gDCGTMFetcherStaticCookieStorage = [[cookieStorageClass alloc] init];
    }
  }
}
...
- (id)initWithRequest:(NSURLRequest *)request {
  ...
  if (self) {
    ...
    if (gDCGTMFetcherStaticCookieStorage != nil) {
      ...
      [self setCookieStorageMethod:kDCGTMHTTPFetcherCookieStorageMethodStatic];
    } else {
      // Default to system default cookie storage
      [self setCookieStorageMethod:kDCGTMHTTPFetcherCookieStorageMethodSystemDefault];
    }
...
- (BOOL)beginFetchWithDelegate:(id)delegate
  ...
  DCGTMAssertSelectorNilOrImplementedWithArgs(delegate, finishedSelector, @encode(DCGTMHTTPFetcher *), @encode(NSData *), @encode(NSError *), 0);
  DCGTMAssertSelectorNilOrImplementedWithArgs(delegate, receivedDataSel_, @encode(DCGTMHTTPFetcher *), @encode(NSData *), 0);
  DCGTMAssertSelectorNilOrImplementedWithArgs(delegate, retrySel_, @encode(DCGTMHTTPFetcher *), @encode(BOOL), @encode(NSError *), 0);
...
- (BOOL)beginFetchMayDelay:(BOOL)mayDelay
  ...
  if (isPreIOS6 && delegateQueue) {
    NSLog(@"DCGTMHTTPFetcher delegateQueue not safe in iOS 5");
  }
#endif
  ...
  [defaultNC postNotificationName:kDCGTMHTTPFetcherStartedNotification
                           object:self];
  return YES;
...
- (void)failToBeginFetchWithError:(NSError *)error {
  if (error == nil) {
    error = [NSError errorWithDomain:kDCGTMHTTPFetcherErrorDomain
                                code:kDCGTMHTTPFetcherErrorDownloadFailed
                            userInfo:nil];
  }
...
- (void)backgroundFetchExpired {
  ...
  NSError *error = [NSError errorWithDomain:kDCGTMHTTPFetcherErrorDomain
                                       code:kDCGTMHTTPFetcherErrorBackgroundExpiration
                                   userInfo:nil];
  [self invokeFetchCallbacksOnDelegateQueueWithData:nil
                                              error:error];
  @synchronized(self) {
...
#endif // DCGTM_BACKGROUND_FETCHING
...
- (void)addCookiesToRequest:(NSMutableURLRequest *)request {
  ...
  if (cookieStorageMethod_ != kDCGTMHTTPFetcherCookieStorageMethodSystemDefault
      && cookieStorageMethod_ != kDCGTMHTTPFetcherCookieStorageMethodNone) {
...
- (void)stopFetchReleasingCallbacks:(BOOL)shouldReleaseCallbacks {
  id <DCGTMHTTPFetcherServiceProtocol> service;
...
- (void)sendStopNotificationIfNeeded {
  ...
  if (sendNow) {
    NSNotificationCenter *defaultNC = [NSNotificationCenter defaultCenter];
    [defaultNC postNotificationName:kDCGTMHTTPFetcherStoppedNotification
                             object:self];
  }
}
...
didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
  @synchronized(self) {
    ...
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:challenge
                                                         forKey:kDCGTMHTTPFetcherErrorChallengeKey];
    NSError *error = [NSError errorWithDomain:kDCGTMHTTPFetcherErrorDomain
                                         code:kDCGTMHTTPFetcherErrorAuthenticationChallengeFailed
                                     userInfo:userInfo];
    ...
    if (downloadFileHandle_ != nil) {
      ...
      @catch (NSException *exc) {
        ...
        NSError *error = [NSError errorWithDomain:kDCGTMHTTPFetcherStatusDomain
                                             code:kDCGTMHTTPFetcherErrorFileHandleException
                                         userInfo:userInfo];
        [self connection:connection didFailWithError:error];
        return;
      }
...
- (NSData *)cachedDataForStatus {
  if ([self statusCode] == kDCGTMHTTPFetcherStatusNotModified
  ...
  if (cachedData) {
    ...
    if (downloadFileHandle_ != nil) {
      ...
      @catch (NSException *) {
        ...
        status = kDCGTMHTTPFetcherErrorFileHandleException;
      }
    } else {
      ...
      if ([self shouldRetryNowForStatus:status error:nil]) {
        ...
        if ([downloadedData_ length] > 0) {
          userInfo = [NSDictionary dictionaryWithObject:downloadedData_
                                                 forKey:kDCGTMHTTPFetcherStatusDataKey];
        }
...
- (BOOL)isRetryError:(NSError *)error {
  ...
  struct retryRecord retries[] = {
    { kDCGTMHTTPFetcherStatusDomain, 408 }, // request timeout
    { kDCGTMHTTPFetcherStatusDomain, 503 }, // service unavailable
    { kDCGTMHTTPFetcherStatusDomain, 504 }, // request timeout
...
- (BOOL)shouldRetryNowForStatus:(NSInteger)status
  ...
  BOOL isFirstAuthError = (authorizer_ != nil)
    ...
    && (status == kDCGTMHTTPFetcherStatusUnauthorized); // 401
...
- (void)beginRetryTimer {
  ...
  [nc postNotificationName:kDCGTMHTTPFetcherRetryDelayStartedNotification
                    object:self];
}
...
- (void)destroyRetryTimer {
  ...
  if (shouldNotify) {
    NSNotificationCenter *defaultNC = [NSNotificationCenter defaultCenter];
    [defaultNC postNotificationName:kDCGTMHTTPFetcherRetryDelayStoppedNotification
                             object:self];
  }
}
...
- (void)setCookieStorageMethod:(NSInteger)method {
  ...
  if (method == kDCGTMHTTPFetcherCookieStorageMethodSystemDefault) {
    // System default
    ...
    if (method == kDCGTMHTTPFetcherCookieStorageMethodStatic) {
      // Store cookies in the static array
      NSAssert(gDCGTMFetcherStaticCookieStorage != nil,
               @"cookie storage requires DCGTMHTTPFetchHistory");

      self.cookieStorage = gDCGTMFetcherStaticCookieStorage;
    } else if (method == kDCGTMHTTPFetcherCookieStorageMethodFetchHistory) {
      // store cookies in the fetch history
      self.cookieStorage = [fetchHistory_ cookieStorage];
    } else {
      // kDCGTMHTTPFetcherCookieStorageMethodNone - ignore cookies
      self.cookieStorage = nil;
    }
  }
}

+ (id <DCGTMCookieStorageProtocol>)staticCookieStorage {
  return gDCGTMFetcherStaticCookieStorage;
}
...
- (id <DCGTMHTTPFetchHistoryProtocol>)fetchHistory {
  return fetchHistory_;
}

- (void)setFetchHistory:(id <DCGTMHTTPFetchHistoryProtocol>)fetchHistory {
  ...
  if (fetchHistory_ != nil) {
    ...
    [self setCookieStorageMethod:kDCGTMHTTPFetcherCookieStorageMethodFetchHistory];

  } else {
    // The fetch history was removed
    if (cookieStorageMethod_ == kDCGTMHTTPFetcherCookieStorageMethodFetchHistory) {
...
+ (Class)connectionClass {
  if (gDCGTMFetcherConnectionClass == nil) {
    gDCGTMFetcherConnectionClass = [NSURLConnection class];
  }
  return gDCGTMFetcherConnectionClass;
}

+ (void)setConnectionClass:(Class)theClass {
  gDCGTMFetcherConnectionClass = theClass;
}
...
void DCGTMAssertSelectorNilOrImplementedWithArgs(id obj, SEL sel, ...) {
...
NSString *DCGTMCleanedUserAgentString(NSString *str) {
...
NSString *DCGTMSystemVersionString(void) {
  ...
  if (savedSystemString == nil) {
    ...
    NSString *model = DCGTMCleanedUserAgentString(rawModel);
...
NSString *DCGTMApplicationIdentifier(NSBundle *bundle) {
  ...
  identifier = DCGTMCleanedUserAgentString(identifier);
  ...
  // Clean up whitespace and special characters
  version = DCGTMCleanedUserAgentString(version);
