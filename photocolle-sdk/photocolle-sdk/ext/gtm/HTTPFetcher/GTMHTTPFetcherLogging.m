...
#ifndef SKIP_DCGTM_FETCH_LOGGING_SNIPPING
#define SKIP_DCGTM_FETCH_LOGGING_SNIPPING 0
#endif

// If DCGTMReadMonitorInputStream is available, it can be used for
// capturing uploaded streams of data
//
// We locally declare methods of DCGTMReadMonitorInputStream so we
// do not need to import the header, as some projects may not have it available
#ifndef DCGTM_NSSTREAM_DELEGATE
@interface DCGTMReadMonitorInputStream : NSInputStream
...
@interface DCGTMFetcherSBJSON
- (void)setHumanReadable:(BOOL)flag;
- (NSString*)stringWithObject:(id)value error:(NSError**)error;
- (id)objectWithString:(NSString*)jsonrep error:(NSError**)error;
@end

@interface DCGTMHTTPFetcher (DCGTMHTTPFetcherLoggingUtilities)
...
@implementation DCGTMHTTPFetcher (DCGTMHTTPFetcherLogging)
...
+ (NSString *)loggingDirectory {
  ...
  if (!gLoggingDirectoryPath) {
    NSArray *arr = nil;
#if DCGTM_IPHONE && TARGET_IPHONE_SIMULATOR
    // default to a directory called DCGTMHTTPDebugLogs into a sandbox-safe
...
#else
    // default to a directory called DCGTMHTTPDebugLogs in the desktop folder
    arr = NSSearchPathForDirectoriesInDomains(NSDesktopDirectory,
                                              NSUserDomainMask, YES);
#endif

    if ([arr count] > 0) {
      NSString *const kDCGTMLogFolderName = @"DCGTMHTTPDebugLogs";

      NSString *desktopPath = [arr objectAtIndex:0];
      NSString *logsFolderPath = [desktopPath stringByAppendingPathComponent:kDCGTMLogFolderName];
...
#if !DCGTM_FOUNDATION_ONLY && !DCGTM_SKIP_LOG_XMLFORMAT
...
- (void)setupStreamLogging {
  ...
  if ([DCGTMHTTPFetcher isLoggingEnabled]
    ...
    if (!didCapture) {
      ...
      // DCGTMReadMonitorInputStream be available
      NSString const *str = @"<<Uploaded stream log unavailable without DCGTMReadMonitorInputStream>>";
...
- (void)logFetchWithError:(NSError *)error {
  ...
  if (gIsLoggingToFile) {
    ...
    if (!gReportedLoggingPath) {
      ...
      NSLog(@"DCGTMHTTPFetcher logging to \"%@\"", parentDir);
    }
#endif
  }
}
...
- (BOOL)logCapturePostStream {
  ...
  // If the class DCGTMReadMonitorInputStream is not available, bail now, since
  // we cannot capture this upload stream
  Class monitorClass = NSClassFromString(@"DCGTMReadMonitorInputStream");
  ...
  [(DCGTMReadMonitorInputStream *)postStream_ setReadDelegate:self];
  [(DCGTMReadMonitorInputStream *)postStream_ setRunLoopModes:[self runLoopModes]];

  SEL readSel = @selector(inputStream:readIntoBuffer:length:);
  [(DCGTMReadMonitorInputStream *)postStream_ setReadSelector:readSel];
...
@implementation DCGTMHTTPFetcher (DCGTMHTTPFetcherLoggingUtilities)
...
+ (NSString *)snipSubstringOfString:(NSString *)originalStr
                 betweenStartString:(NSString *)startStr
                          endString:(NSString *)endStr {
#if SKIP_DCGTM_FETCH_LOGGING_SNIPPING
  return originalStr;
#else
  ...
  NSString *result = [originalStr stringByReplacingCharactersInRange:replaceRange
                                                          withString:@"_snip_"];
  return result;
#endif // SKIP_DCGTM_FETCH_LOGGING_SNIPPING
}
...
+ (id)JSONObjectWithData:(NSData *)data {
  ...
  if (serializer) {
    ...
    if (jsonParseClass) {
      DCGTMFetcherSBJSON *parser = [[[jsonParseClass alloc] init] autorelease];
...
+ (id)stringWithJSONObject:(id)obj {
  ...
  if (serializer) {
    ...
    if (jsonWriterClass) {
      DCGTMFetcherSBJSON *writer = [[[jsonWriterClass alloc] init] autorelease];
