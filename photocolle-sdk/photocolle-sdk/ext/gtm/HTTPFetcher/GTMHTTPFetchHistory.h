...
//
// Users of the DCGTMHTTPFetcher class may optionally create and set a fetch
...
extern const NSUInteger kDCGTMDefaultETaggedDataCacheMemoryCapacity;
...
@class DCGTMURLCache;
@class DCGTMCookieStorage;

@interface DCGTMHTTPFetchHistory : NSObject <DCGTMHTTPFetchHistoryProtocol> {
 @private
  DCGTMURLCache *etaggedDataCache_;
  BOOL shouldRememberETags_;
  BOOL shouldCacheETaggedData_;        // if NO, then only headers are cached
  DCGTMCookieStorage *cookieStorage_;
}
...
// the default ETag data cache capacity is kDCGTMDefaultETaggedDataCacheMemoryCapacity
@property (assign) NSUInteger memoryCapacity;

@property (retain) DCGTMCookieStorage *cookieStorage;
...
// DCGTMURLCache and DCGTMCachedURLResponse have interfaces similar to their
...
@interface DCGTMCachedURLResponse : NSObject {
 @private
...
@interface DCGTMURLCache : NSObject {
  NSMutableDictionary *responses_; // maps request URL to DCGTMCachedURLResponse
...
- (void)storeCachedResponse:(DCGTMCachedURLResponse *)cachedResponse forRequest:(NSURLRequest *)request;
...
@interface DCGTMCookieStorage : NSObject <DCGTMCookieStorageProtocol> {
 @private
