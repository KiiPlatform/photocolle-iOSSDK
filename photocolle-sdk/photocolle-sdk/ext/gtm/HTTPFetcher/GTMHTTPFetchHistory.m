...
//
//  DCGTMHTTPFetchHistory.m
//

#import "GTMHTTPFetchHistory.h"

const NSTimeInterval kCachedURLReservationInterval = 60.0; // 1 minute
static NSString* const kDCGTMIfNoneMatchHeader = @"If-None-Match";
static NSString* const kDCGTMETagHeader = @"Etag";

#if DCGTM_IPHONE
// iPhone: up to 1MB memory
const NSUInteger kDCGTMDefaultETaggedDataCacheMemoryCapacity = 1 * 1024 * 1024;
#else
// Mac OS X: up to 15MB memory
const NSUInteger kDCGTMDefaultETaggedDataCacheMemoryCapacity = 15 * 1024 * 1024;
#endif


@implementation DCGTMCookieStorage
...
//
// DCGTMCachedURLResponse
//

@implementation DCGTMCachedURLResponse
...
- (NSComparisonResult)compareUseDate:(DCGTMCachedURLResponse *)other {
...
@implementation DCGTMURLCache

@dynamic memoryCapacity;

- (id)init {
  return [self initWithMemoryCapacity:kDCGTMDefaultETaggedDataCacheMemoryCapacity];
}
...
- (void)pruneCacheResponses {
  ...
  for (NSURL *key in sortedKeys) {
    DCGTMCachedURLResponse *response = [responses_ objectForKey:key];
...
- (void)storeCachedResponse:(DCGTMCachedURLResponse *)cachedResponse
...
- (DCGTMCachedURLResponse *)cachedResponseForRequest:(NSURLRequest *)request {
  DCGTMCachedURLResponse *response;
...
- (id)initWithMemoryCapacity:(NSUInteger)totalBytes
  ...
  self = [super init];
  if (self != nil) {
    etaggedDataCache_ = [[DCGTMURLCache alloc] initWithMemoryCapacity:totalBytes];
    shouldRememberETags_ = shouldCacheETaggedData;
    shouldCacheETaggedData_ = shouldCacheETaggedData;
    cookieStorage_ = [[DCGTMCookieStorage alloc] init];
  }
  return self;
}
  ...
  @synchronized(self) {
    if ([self shouldRememberETags]) {
      ...
      if (isHTTPGet) {
        ...
        // request
        NSString *specifiedETag = [request valueForHTTPHeaderField:kDCGTMIfNoneMatchHeader];
        if (specifiedETag == nil) {
          ...
          if (cachedETag != nil) {
            [request addValue:cachedETag forHTTPHeaderField:kDCGTMIfNoneMatchHeader];
          }
        } else {
...
- (void)updateFetchHistoryWithRequest:(NSURLRequest *)request
  ...
  @synchronized(self) {
    ...
    if (statusCode != kDCGTMHTTPFetcherStatusNotModified) {
      ...
      NSString* etag = [headers objectForKey:kDCGTMETagHeader];
      ...
      if (etag != nil && statusCode < 300) {
        ...
        DCGTMCachedURLResponse *cachedResponse;
        cachedResponse = [[[DCGTMCachedURLResponse alloc] initWithResponse:response
...
- (NSString *)cachedETagForRequest:(NSURLRequest *)request {
  ...
  NSDictionary *headers = [(NSHTTPURLResponse *)response allHeaderFields];
  NSString *cachedETag = [headers objectForKey:kDCGTMETagHeader];
  if (cachedETag) {
