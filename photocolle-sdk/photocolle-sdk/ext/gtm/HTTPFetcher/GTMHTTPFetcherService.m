...
//
//  DCGTMHTTPFetcherService.m
//

#import "GTMHTTPFetcherService.h"

@interface DCGTMHTTPFetcher (ServiceMethods)
- (BOOL)beginFetchMayDelay:(BOOL)mayDelay
              mayAuthorize:(BOOL)mayAuthorize;
@end

@interface DCGTMHTTPFetcherService ()
...
@implementation DCGTMHTTPFetcherService
...
- (id)init {
  self = [super init];
  if (self) {
    fetchHistory_ = [[DCGTMHTTPFetchHistory alloc] init];
    delayedHosts_ = [[NSMutableDictionary alloc] init];
    runningHosts_ = [[NSMutableDictionary alloc] init];
    cookieStorageMethod_ = kDCGTMHTTPFetcherCookieStorageMethodFetchHistory;
...
- (id)fetcherWithRequest:(NSURLRequest *)request
  ...
  DCGTMHTTPFetcher *fetcher = [fetcherClass fetcherWithRequest:request];
...
- (DCGTMHTTPFetcher *)fetcherWithRequest:(NSURLRequest *)request {
  return [self fetcherWithRequest:request
                     fetcherClass:[DCGTMHTTPFetcher class]];
}
...
- (void)addRunningFetcher:(DCGTMHTTPFetcher *)fetcher
...
- (void)addDelayedFetcher:(DCGTMHTTPFetcher *)fetcher
                  forHost:(NSString *)host {
...
// the fetcher
- (void)performSelector:(SEL)sel onStartThreadForFetcher:(DCGTMHTTPFetcher *)fetcher {
...
- (void)startFetcherOnCurrentThread:(DCGTMHTTPFetcher *)fetcher {
  [fetcher beginFetchMayDelay:NO
                 mayAuthorize:YES];
}

- (void)startFetcher:(DCGTMHTTPFetcher *)fetcher {
  [self performSelector:@selector(startFetcherOnCurrentThread:)
    onStartThreadForFetcher:fetcher];
}

- (void)stopFetcherOnCurrentThread:(DCGTMHTTPFetcher *)fetcher {
  [fetcher stopFetching];
}

- (void)stopFetcher:(DCGTMHTTPFetcher *)fetcher {
...
- (void)fetcherDidStop:(DCGTMHTTPFetcher *)fetcher {
  ...
  @synchronized(self) {
    ...
    while ([delayedForHost count] > 0
      ...
      DCGTMHTTPFetcher *nextFetcher = nil;
      for (DCGTMHTTPFetcher *delayedFetcher in delayedForHost) {
        if (nextFetcher == nil
...
- (NSArray *)issuedFetchersWithRequestURL:(NSURL *)requestURL {
  @synchronized(self) {
    ...
    for (DCGTMHTTPFetcher *fetcher in runningForHost) {
    ...
    NSArray *delayedForHost = [delayedHosts_ objectForKey:host];
    for (DCGTMHTTPFetcher *fetcher in delayedForHost) {
      NSURL *fetcherURL = [[[fetcher mutableRequest] URL] absoluteURL];
      if ([fetcherURL isEqual:absRequestURL]) {
        if (array == nil) {
          array = [NSMutableArray array];
...
- (void)detachAuthorizer {
  ...
  if ([authorizer_ respondsToSelector:@selector(fetcherService)]) {
    DCGTMHTTPFetcherService *authFS = [authorizer_ fetcherService];
    if (authFS == self) {
