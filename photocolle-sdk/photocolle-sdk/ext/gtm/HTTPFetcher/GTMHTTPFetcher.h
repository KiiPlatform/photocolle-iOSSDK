...
//  DCGTMHTTPFetcher* myFetcher = [DCGTMHTTPFetcher fetcherWithRequest:request];
//
...
//
//  - (void)myFetcher:(DCGTMHTTPFetcher *)fetcher finishedWithData:(NSData *)retrievedData error:(NSError *)error;
//
...
//        kDCGTMHTTPFetcherStatusDomain and code set to the server status.
//
...
//     = [[error userInfo] objectForKey:kDCGTMHTTPFetcherErrorChallengeKey];
...
//
// By default, DCGTMHTTPFetcher uses a mutable array held statically to track
...
// setCookieStorageMethod: with kDCGTMHTTPFetcherCookieStorageMethodSystemDefault.
//
...
// data.
//
// To get this behavior, create the fetcher from an DCGTMHTTPFetcherService object
// and look for a fetch callback error with code 304
// (kDCGTMHTTPFetcherStatusNotModified) like this:
//
// - (void)myFetcher:(DCGTMHTTPFetcher *)fetcher finishedWithData:(NSData *)data error:(NSError *)error {
//    if ([error code] == kDCGTMHTTPFetcherStatusNotModified) {
...
//
//  - (void)myFetcher:(DCGTMHTTPFetcher *)fetcher receivedData:(NSData *)dataReceivedSoFar;
//
...
//   -(BOOL)fetcher:(DCGTMHTTPFetcher *)fetcher willRetry:(BOOL)suggestedWillRetry forError:(NSError *)error
...
// error argument, and have the domain kDCGTMHTTPFetcherStatusDomain. The
// user's method may look something like this:
//
//  -(BOOL)myFetcher:(DCGTMHTTPFetcher *)fetcher willRetry:(BOOL)suggestedWillRetry forError:(NSError *)error {
//
...
#else
  #if TARGET_OS_IPHONE
    #ifndef DCGTM_FOUNDATION_ONLY
      #define DCGTM_FOUNDATION_ONLY 1
    #endif
    #ifndef DCGTM_IPHONE
      #define DCGTM_IPHONE 1
    #endif
  #endif
#endif

#if TARGET_OS_IPHONE && (__IPHONE_OS_VERSION_MAX_ALLOWED >= 40000)
  #define DCGTM_BACKGROUND_FETCHING 1
#endif
...
extern NSString *const kDCGTMHTTPFetcherStartedNotification;
extern NSString *const kDCGTMHTTPFetcherStoppedNotification;
extern NSString *const kDCGTMHTTPFetcherRetryDelayStartedNotification;
extern NSString *const kDCGTMHTTPFetcherRetryDelayStoppedNotification;

// callback constants
extern NSString *const kDCGTMHTTPFetcherErrorDomain;
extern NSString *const kDCGTMHTTPFetcherStatusDomain;
extern NSString *const kDCGTMHTTPFetcherErrorChallengeKey;
extern NSString *const kDCGTMHTTPFetcherStatusDataKey;  // data returned with a kDCGTMHTTPFetcherStatusDomain error
...
enum {
  kDCGTMHTTPFetcherErrorDownloadFailed = -1,
  kDCGTMHTTPFetcherErrorAuthenticationChallengeFailed = -2,
  kDCGTMHTTPFetcherErrorChunkUploadFailed = -3,
  kDCGTMHTTPFetcherErrorFileHandleException = -4,
  kDCGTMHTTPFetcherErrorBackgroundExpiration = -6,

  // The code kDCGTMHTTPFetcherErrorAuthorizationFailed (-5) has been removed;
  // look for status 401 instead.

  kDCGTMHTTPFetcherStatusNotModified = 304,
  kDCGTMHTTPFetcherStatusBadRequest = 400,
  kDCGTMHTTPFetcherStatusUnauthorized = 401,
  kDCGTMHTTPFetcherStatusForbidden = 403,
  kDCGTMHTTPFetcherStatusPreconditionFailed = 412
};

// cookie storage methods
enum {
  kDCGTMHTTPFetcherCookieStorageMethodStatic = 0,
  kDCGTMHTTPFetcherCookieStorageMethodFetchHistory = 1,
  kDCGTMHTTPFetcherCookieStorageMethodSystemDefault = 2,
  kDCGTMHTTPFetcherCookieStorageMethodNone = 3
};
...
@class DCGTMHTTPFetcher;

@protocol DCGTMCookieStorageProtocol <NSObject>
// This protocol allows us to call into the service without requiring
// DCGTMCookieStorage sources in this project
//
// The public interface for cookie handling is the DCGTMCookieStorage class,
...
@protocol DCGTMHTTPFetchHistoryProtocol <NSObject>
// This protocol allows us to call the fetch history object without requiring
// DCGTMHTTPFetchHistory sources in this project
...
- (id <DCGTMCookieStorageProtocol>)cookieStorage;
...
@protocol DCGTMHTTPFetcherServiceProtocol <NSObject>
// This protocol allows us to call into the service without requiring
// DCGTMHTTPFetcherService sources in this project

@property (retain) NSOperationQueue *delegateQueue;

- (BOOL)fetcherShouldBeginFetching:(DCGTMHTTPFetcher *)fetcher;
- (void)fetcherDidStop:(DCGTMHTTPFetcher *)fetcher;

- (DCGTMHTTPFetcher *)fetcherWithRequest:(NSURLRequest *)request;
- (BOOL)isDelayingFetcher:(DCGTMHTTPFetcher *)fetcher;
@end
...
// DCGTMHTTPFetcher objects are used for async retrieval of an http get or post
//
// See additional comments at the beginning of this file
@interface DCGTMHTTPFetcher : NSObject {
 @protected
  ...
  id <DCGTMHTTPFetchHistoryProtocol> fetchHistory_; // if supplied by the caller, used for Last-Modified-Since checks and cookies
  NSInteger cookieStorageMethod_;   // constant from above
  id <DCGTMCookieStorageProtocol> cookieStorage_;

  id <DCGTMFetcherAuthorizationProtocol> authorizer_;

  // the service object that created and monitors this fetcher, if any
  id <DCGTMHTTPFetcherServiceProtocol> service_;
...
// The default cookie storage method is kDCGTMHTTPFetcherCookieStorageMethodStatic
// without a fetch history set, and kDCGTMHTTPFetcherCookieStorageMethodFetchHistory
// with a fetch history set
//
// Applications needing control of cookies across a sequence of fetches should
// create fetchers from a DCGTMHTTPFetcherService object (which encapsulates
...
@property (retain) id <DCGTMFetcherAuthorizationProtocol> authorizer;

// The service object that created and monitors this fetcher, if any
@property (retain) id <DCGTMHTTPFetcherServiceProtocol> service;
...
//  - (void)myFetcher:(DCGTMHTTPFetcher *)fetcher
...
//   - (void)fetcher:(DCGTMHTTPFetcher *)fetcher finishedWithData:(NSData *)data error:(NSError *)error;
//
...
// is set by a DCGTMFetcherService object when it creates a fetcher.
//
// Side effect: setting fetch history implicitly calls setCookieStorageMethod:
@property (retain) id <DCGTMHTTPFetchHistoryProtocol> fetchHistory;
...
// Users who wish to replace DCGTMHTTPFetcher's use of NSURLConnection
