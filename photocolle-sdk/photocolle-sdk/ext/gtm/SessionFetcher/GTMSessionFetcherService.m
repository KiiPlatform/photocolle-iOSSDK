...
NSString *const kDCGTMSessionFetcherServiceSessionBecameInvalidNotification
    = @"kDCGTMSessionFetcherServiceSessionBecameInvalidNotification";
NSString *const kDCGTMSessionFetcherServiceSessionKey
    = @"kDCGTMSessionFetcherServiceSessionKey";

@interface DCGTMSessionFetcher (ServiceMethods)
...
@interface DCGTMSessionFetcherService ()
...
// fetcher.
@interface DCGTMSessionFetcherSessionDelegateDispatcher : NSObject<NSURLSessionDelegate>
...
- (instancetype)initWithParentService:(DCGTMSessionFetcherService *)parentService
               sessionDiscardInterval:(NSTimeInterval)discardInterval;

- (void)setFetcher:(DCGTMSessionFetcher *)fetcher
           forTask:(NSURLSessionTask *)task;
- (void)removeFetcher:(DCGTMSessionFetcher *)fetcher;
...
@implementation DCGTMSessionFetcherService {
  ...
  DCGTMSessionFetcherSessionDelegateDispatcher *_delegateDispatcher;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _delegateDispatcher =
        [[DCGTMSessionFetcherSessionDelegateDispatcher alloc] initWithParentService:self
    ...
    _userAgent = DCGTMFetcherStandardUserAgentString(nil);
#endif
  }
  return self;
}
...
- (id)fetcherWithRequest:(NSURLRequest *)request
  ...
  DCGTMSessionFetcher *fetcher = [[fetcherClass alloc] initWithRequest:request
...
- (DCGTMSessionFetcher *)fetcherWithRequest:(NSURLRequest *)request {
  return [self fetcherWithRequest:request
                     fetcherClass:[DCGTMSessionFetcher class]];
}
...
- (NSURLSession *)sessionForFetcherCreation {
  ...
  @synchronized(self) {
    ...
    DCGTMSESSION_ASSERT_DEBUG(semaphore != nil || _delegateDispatcher == nil, @"Expected semaphore");
  }
  if (semaphore) {
  ...
  @synchronized(self) {
    NSURLSession *session = _delegateDispatcher.session;
    if (session) {
      DCGTMSESSION_ASSERT_DEBUG(semaphore == _delegateDispatcher.sessionCreationSemaphore,
...
- (void)addRunningFetcher:(DCGTMSessionFetcher *)fetcher
...
- (void)addDelayedFetcher:(DCGTMSessionFetcher *)fetcher
                  forHost:(NSString *)host {
...
- (BOOL)fetcherShouldBeginFetching:(DCGTMSessionFetcher *)fetcher {
  ...
  @synchronized(self) {
    ...
    if ([host length] == 0) {
      ...
      DCGTMSESSION_ASSERT_DEBUG([[requestURL scheme] isEqual:@"data"], @"%@ lacks host", fetcher);
      return YES;
    }
    ...
    if (runningForHost != nil
      ...
      DCGTMSESSION_ASSERT_DEBUG(NO, @"%@ was already running", fetcher);
      return YES;
    }
...
- (void)startFetcher:(DCGTMSessionFetcher *)fetcher {
...
- (DCGTMSessionFetcherSessionDelegateDispatcher *)delegateDispatcherForFetcher:(DCGTMSessionFetcher *)fetcher {
  ...
  if (fetcherSession) {
    ...
    if (hasDispatcher) {
      DCGTMSESSION_ASSERT_DEBUG([fetcherDelegate isKindOfClass:[DCGTMSessionFetcherSessionDelegateDispatcher class]],
                              @"Fetcher delegate class: %@", [fetcherDelegate class]);
      return (DCGTMSessionFetcherSessionDelegateDispatcher *)fetcherDelegate;
    }
  }
  return nil;
}
...
- (void)fetcherDidCreateSession:(DCGTMSessionFetcher *)fetcher {
  if (fetcher.canShareSession) {
    NSURLSession *fetcherSession = fetcher.session;
    DCGTMSESSION_ASSERT_DEBUG(fetcherSession != nil, @"Fetcher missing its session: %@", fetcher);

    DCGTMSessionFetcherSessionDelegateDispatcher *delegateDispatcher =
        [self delegateDispatcherForFetcher:fetcher];
    if (delegateDispatcher) {
      DCGTMSESSION_ASSERT_DEBUG(delegateDispatcher.session == nil,
...
- (void)fetcherDidBeginFetching:(DCGTMSessionFetcher *)fetcher {
  ...
  DCGTMSessionFetcherSessionDelegateDispatcher *delegateDispatcher =
      [self delegateDispatcherForFetcher:fetcher];
  if (delegateDispatcher) {
    DCGTMSESSION_ASSERT_DEBUG(fetcher.canShareSession,
    ...
    NSURLSession *fetcherSession = fetcher.session;
    DCGTMSESSION_ASSERT_DEBUG(sharedSession != nil, @"Missing delegate session: %@", fetcher);
    DCGTMSESSION_ASSERT_DEBUG(fetcherSession == sharedSession,
    ...
    if (sharedSession != nil && fetcherSession == sharedSession) {
      ...
      DCGTMSESSION_ASSERT_DEBUG(task != nil, @"Missing session task: %@", fetcher);
...
- (void)stopFetcher:(DCGTMSessionFetcher *)fetcher {
...
- (void)fetcherDidStop:(DCGTMSessionFetcher *)fetcher {
  ...
  @synchronized(self) {
    ...
    while ([delayedForHost count] > 0
      ...
      DCGTMSessionFetcher *nextFetcher = nil;
      for (DCGTMSessionFetcher *delayedFetcher in delayedForHost) {
        if (nextFetcher == nil
  ...
  for (DCGTMSessionFetcher *nextFetcher in fetchersToStart) {
...
- (NSArray *)issuedFetchers {
  @synchronized(self) {
    ...
    DCGTMSESSION_ASSERT_DEBUG([allFetchers count] == [[NSSet setWithArray:allFetchers] count],
...
- (void)stopAllFetchers {
  @synchronized(self) {
    ...
    for (NSArray *delayedForHost in delayedFetchersByHost) {
      for (DCGTMSessionFetcher *fetcher in delayedForHost) {
    ...
    for (NSArray *runningForHost in runningFetchersByHost) {
      for (DCGTMSessionFetcher *fetcher in runningForHost) {
        [self stopFetcher:fetcher];
      }
    }
  }
...
- (id<DCGTMFetcherAuthorizationProtocol>)authorizer {
  @synchronized(self) {
    return _authorizer;
  }
}

- (void)setAuthorizer:(id<DCGTMFetcherAuthorizationProtocol>)obj {
  @synchronized(self) {
    if (obj != _authorizer) {
      [self detachAuthorizer];
    }
...
- (NSOperationQueue *)delegateQueue {
  // Provided for compatibility with the old fetcher service.  The DCGTM-oauth2 code respects
...
@implementation DCGTMSessionFetcherService (TestingSupport)
...
+ (instancetype)mockFetcherServiceWithFakedData:(NSData *)fakedDataOrNil
  ...
  NSHTTPURLResponse *fakedResponse =
      [[NSHTTPURLResponse alloc] initWithURL:[NSURL URLWithString:@"http://example.invalid"]
                                ...
                                headerFields:nil];
  DCGTMSessionFetcherService *service = [[self alloc] init];
  service.allowedInsecureSchemes = @[ @"http" ];
  service.testBlock = ^(DCGTMSessionFetcher *fetcherToTest,
                        DCGTMSessionFetcherTestResponse testResponse) {
    testResponse(fakedResponse, fakedDataOrNil, fakedErrorOrNil);
  };
  return service;
#else
  DCGTMSESSION_ASSERT_DEBUG(0, @"Test blocks disabled");
  return nil;
...
- (BOOL)waitForCompletionOfAllFetchersWithTimeout:(NSTimeInterval)timeoutInSeconds {
  ...
  while (([self numberOfFetchers] > 0 || [_stoppedFetchersToWaitFor count] > 0)) {
    ...
    DCGTMSessionFetcher *stoppedFetcher = [_stoppedFetchersToWaitFor firstObject];
    if (stoppedFetcher) {
...
@implementation DCGTMSessionFetcherService (BackwardsCompatibilityOnly)
...
@implementation DCGTMSessionFetcherSessionDelegateDispatcher {
  __weak DCGTMSessionFetcherService *_parentService;
  NSURLSession *_session;
  dispatch_semaphore_t _sessionCreationSemaphore;

  // The task map maps NSURLSessionTasks to DCGTMSessionFetchers
...
- (void)discardTimerFired:(NSTimer *)timer {
  DCGTMSessionFetcherService *service;
  @synchronized(self) {
...
- (void)setFetcher:(DCGTMSessionFetcher *)fetcher forTask:(NSURLSessionTask *)task {
  DCGTMSESSION_ASSERT_DEBUG(fetcher != nil, @"missing fetcher");
...
- (void)removeFetcher:(DCGTMSessionFetcher *)fetcher {
  @synchronized(self) {
    ...
    DCGTMSESSION_ASSERT_DEBUG([tasks count] <= 1, @"fetcher task not unmapped: %@", tasks);
...
- (void)URLSession:(NSURLSession *)session didBecomeInvalidWithError:(NSError *)error {
  ...
  [localTaskToFetcherMap enumerateKeysAndObjectsUsingBlock:^(NSURLSessionTask *task,
    ...
    if (fetcher.session == session) {
        ...
        DCGTMSESSION_ASSERT_DEBUG(0, @"Unexpected session in fetcher: %@ has %@ (expected %@)",
                                fetcher, fetcher.session, session);
      }
  }];

  // Our tests rely on this notification to know the session discard timer fired.
  NSDictionary *userInfo = @{ kDCGTMSessionFetcherServiceSessionKey : session };
  NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
  [nc postNotificationName:kDCGTMSessionFetcherServiceSessionBecameInvalidNotification
...
didBecomeDownloadTask:(NSURLSessionDownloadTask *)downloadTask {
  id<NSURLSessionDataDelegate> fetcher = [self fetcherForTask:dataTask];
  DCGTMSESSION_ASSERT_DEBUG(fetcher != nil, @"Missing fetcher for %@", dataTask);
  [self removeTaskFromMap:dataTask];
  if (fetcher) {
    DCGTMSESSION_ASSERT_DEBUG([fetcher isKindOfClass:[DCGTMSessionFetcher class]],
                            @"Expecting DCGTMSessionFetcher");
    [self setFetcher:(DCGTMSessionFetcher *)fetcher forTask:downloadTask];
  }
