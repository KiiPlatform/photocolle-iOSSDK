...
#if TARGET_OS_IPHONE && !defined(DCGTM_BACKGROUND_TASK_FETCHING)
#define DCGTM_BACKGROUND_TASK_FETCHING 1
#endif

NSString *const kDCGTMSessionFetcherStartedNotification           = @"kDCGTMSessionFetcherStartedNotification";
NSString *const kDCGTMSessionFetcherStoppedNotification           = @"kDCGTMSessionFetcherStoppedNotification";
NSString *const kDCGTMSessionFetcherRetryDelayStartedNotification = @"kDCGTMSessionFetcherRetryDelayStartedNotification";
NSString *const kDCGTMSessionFetcherRetryDelayStoppedNotification = @"kDCGTMSessionFetcherRetryDelayStoppedNotification";

NSString *const kDCGTMSessionFetcherCompletionInvokedNotification = @"kDCGTMSessionFetcherCompletionInvokedNotification";
NSString *const kDCGTMSessionFetcherCompletionDataKey = @"data";
NSString *const kDCGTMSessionFetcherCompletionErrorKey = @"error";

NSString *const kDCGTMSessionFetcherErrorDomain       = @"com.google.DCGTMSessionFetcher";
NSString *const kDCGTMSessionFetcherStatusDomain      = @"com.google.HTTPStatus";
NSString *const kDCGTMSessionFetcherStatusDataKey     = @"data";  // data returned with a kDCGTMSessionFetcherStatusDomain error

static NSString *const kDCGTMSessionIdentifierPrefix = @"com.google.DCGTMSessionFetcher";
static NSString *const kDCGTMSessionIdentifierDestinationFileURLMetadataKey = @"_destURL";
static NSString *const kDCGTMSessionIdentifierBodyFileURLMetadataKey        = @"_bodyURL";
...
#ifdef DCGTMSESSION_PERSISTED_DESTINATION_KEY
// Projects using unique class names should also define a unique persisted destination key.
static NSString * const kDCGTMSessionFetcherPersistedDestinationKey =
    DCGTMSESSION_PERSISTED_DESTINATION_KEY;
#else
static NSString * const kDCGTMSessionFetcherPersistedDestinationKey =
    @"com.google.DCGTMSessionFetcher.downloads";
#endif
...
#if 0
#define DCGTM_LOG_BACKGROUND_SESSION(...) DCGTMSESSION_LOG_DEBUG(__VA_ARGS__)
#else
#define DCGTM_LOG_BACKGROUND_SESSION(...)
#endif

#ifndef DCGTM_TARGET_SUPPORTS_APP_TRANSPORT_SECURITY
  #if (!TARGET_OS_IPHONE && defined(MAC_OS_X_VERSION_10_11) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_11) \
      || (TARGET_OS_IPHONE && defined(__IPHONE_9_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_9_0)
    #define DCGTM_TARGET_SUPPORTS_APP_TRANSPORT_SECURITY 1
  #endif
#endif

@interface DCGTMSessionFetcher ()
...
@interface DCGTMSessionFetcher (DCGTMSessionFetcherLoggingInternal)
...
static DCGTMSessionFetcherTestBlock gGlobalTestBlock;

@implementation DCGTMSessionFetcher {
...
#if DCGTM_BACKGROUND_TASK_FETCHING
  ...
  id<DCGTMFetcherAuthorizationProtocol> _authorizer;

  // The service object that created and monitors this fetcher, if any
  id<DCGTMSessionFetcherServiceProtocol> _service;
...
+ (instancetype)fetcherWithDownloadResumeData:(NSData *)resumeData {
  DCGTMSessionFetcher *fetcher = [self fetcherWithRequest:nil];
...
+ (instancetype)fetcherWithSessionIdentifier:(NSString *)sessionIdentifier {
  ...
  DCGTMSessionFetcher *fetcher = [sessionIdentifierToFetcherMap objectForKey:sessionIdentifier];
  if (!fetcher && [sessionIdentifier hasPrefix:kDCGTMSessionIdentifierPrefix]) {
...
#if !DCGTM_ALLOW_INSECURE_REQUESTS
+ (BOOL)appAllowsInsecureRequests {
...
#if DCGTM_TARGET_SUPPORTS_APP_TRANSPORT_SECURITY
  ...
  // security checks.
  return YES;
#endif  // DCGTM_TARGET_SUPPORTS_APP_TRANSPORT_SECURITY
}
#else  // DCGTM_ALLOW_INSECURE_REQUESTS
+ (BOOL)appAllowsInsecureRequests {
  return YES;
}
#endif  // !DCGTM_ALLOW_INSECURE_REQUESTS
...
- (instancetype)initWithRequest:(NSURLRequest *)request
  ...
  if (self) {
    if (![NSURLSession class]) {
      Class oldFetcherClass = NSClassFromString(@"DCGTMHTTPFetcher");
      if (oldFetcherClass) {
...
- (void)dealloc {
  DCGTMSESSION_ASSERT_DEBUG(!_isStopNotificationNeeded,
...
- (DCGTMSessionFetcherCompletionHandler)completionHandlerWithTarget:(id)target
  ...
  DCGTMSessionFetcherAssertValidSelector(target, finishedSelector, @encode(DCGTMSessionFetcher *),
                                       @encode(NSData *), @encode(NSError *), 0);
  DCGTMSessionFetcherCompletionHandler completionHandler = ^(NSData *data, NSError *error) {
...
- (void)beginFetchWithDelegate:(id)target
             didFinishSelector:(SEL)finishedSelector {
  DCGTMSessionFetcherCompletionHandler handler =  [self completionHandlerWithTarget:target
...
- (void)beginFetchMayDelay:(BOOL)mayDelay
  ...
  if (_sessionTask != nil) {
    ...
    DCGTMSESSION_ASSERT_DEBUG(NO, @"Fetch object %@ being reused; this should never happen", self);
    [self failToBeginFetchWithError:beginFailureError(DCGTMSessionFetcherErrorDownloadFailed)];
    return;
  }

  NSURL *requestURL = [_request URL];
  if (requestURL == nil && !_downloadResumeData && !_sessionIdentifier) {
    DCGTMSESSION_ASSERT_DEBUG(NO, @"Beginning a fetch requires a request with a URL");
    [self failToBeginFetchWithError:beginFailureError(DCGTMSessionFetcherErrorDownloadFailed)];
    return;
  }
  ...
  if (_bodyFileURL) {
    ...
    if (![_bodyFileURL checkResourceIsReachableAndReturnError:&fileCheckError]) {
      ...
      DCGTMSESSION_ASSERT_DEBUG_OR_LOG(0, @"Body file is unreachable: %@\n  %@",
      ...
      if (!allowRequest) {
        ...
        if (isLocalhostRequest) {
          if (_allowLocalhostRequest) {
            ...
            DCGTMSESSION_ASSERT_DEBUG(NO, @"Fetch request for localhost but fetcher"
                                        @" allowLocalhostRequest is not set: %@", requestURL);
          }
        } else {
          DCGTMSESSION_ASSERT_DEBUG(NO, @"Insecure fetch request has a scheme (%@)"
...
#endif
        [self failToBeginFetchWithError:beginFailureError(DCGTMSessionFetcherErrorInsecureRequest)];
        return;
      }
    }  // !isSecure
  ...
  if (!_session) {
    ...
    DCGTMSESSION_ASSERT_DEBUG(_session, @"Couldn't create session");
    ...
    DCGTMSESSION_ASSERT_DEBUG(_session.delegate == delegate, @"Couldn't assign delegate.");
        ...
        dispatch_after(checkForFeedbackDelay, dispatch_get_main_queue(), ^{
          if (!_sessionTask && !_request) {
            ...
            NSError *sessionError =
                [NSError errorWithDomain:kDCGTMSessionFetcherErrorDomain
                                    code:DCGTMSessionFetcherErrorBackgroundFetchFailed
                                userInfo:nil];
  ...
  if (_downloadResumeData) {
    ...
    DCGTMSESSION_ASSERT_DEBUG_OR_LOG(_sessionTask,
    ...
    DCGTMSESSION_ASSERT_DEBUG_OR_LOG(_sessionTask, @"Failed downloadTaskWithRequest for %@, %@",
                                   _session, _request);
    ...
    if (_bodyFileURL) {
      ...
      DCGTMSESSION_ASSERT_DEBUG_OR_LOG(_bodyData != nil,
    ...
    needsDataAccumulator = YES;
    DCGTMSESSION_ASSERT_DEBUG_OR_LOG(_sessionTask, @"Failed dataTaskWithRequest for %@, %@",
                                   _session, _request);
  }
  ...
  if (!_sessionTask) {
    ...
    [self failToBeginFetchWithError:beginFailureError(DCGTMSessionFetcherErrorTaskCreationFailed)];
    return;
  }
...
#if DCGTM_DISABLE_FETCHER_TEST_BLOCK
  DCGTMSESSION_ASSERT_DEBUG(_testBlock == nil && gGlobalTestBlock == nil, @"test blocks disabled");
  _testBlock = nil;
#else
  if (!_testBlock) {
    if (gGlobalTestBlock) {
...
#endif  // DCGTM_DISABLE_FETCHER_TEST_BLOCK
...
#else
    NSString *bgTaskName = @"DCGTMSessionFetcher";
#endif
  ...
  DCGTMSESSION_ASSERT_DEBUG(!_isStopNotificationNeeded, @"Start notification without a prior stop");
  ...
  [defaultNC postNotificationName:kDCGTMSessionFetcherStartedNotification
                           object:self];
        ...
        _bodyStreamProvider(^(NSInputStream *bodyStream){
          ...
          [self invokeOnCallbackQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0)
                     afterUserStopped:NO
                                block:^{
            NSError *streamError;
            NSData *streamedData = DCGTMDataFromInputStream(bodyStream, &streamError);
...
- (void)simulateByteTransferReportWithDataLength:(int64_t)totalDataLength
                                           block:(DCGTMSessionFetcherSendProgressBlock)block {
...
+ (NSUserDefaults *)fetcherUserDefaults {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    Class fetcherUserDefaultsClass = NSClassFromString(@"DCGTMSessionFetcherUserDefaultsFactory");
...
- (void)addPersistedBackgroundSessionToDefaults {
  ...
  DCGTM_LOG_BACKGROUND_SESSION(@"Add to background sessions: %@", newBackgroundSessions);

  NSUserDefaults *userDefaults = [[self class] fetcherUserDefaults];
  [userDefaults setObject:newBackgroundSessions
                   forKey:kDCGTMSessionFetcherPersistedDestinationKey];
...
- (void)removePersistedBackgroundSessionFromDefaults {
  ...
  DCGTM_LOG_BACKGROUND_SESSION(@"Remove from background sessions: %@", newBackgroundSessions);

  NSUserDefaults *userDefaults = [[self class] fetcherUserDefaults];
  if ([newBackgroundSessions count] == 0) {
    [userDefaults removeObjectForKey:kDCGTMSessionFetcherPersistedDestinationKey];
  } else {
    [userDefaults setObject:newBackgroundSessions
                     forKey:kDCGTMSessionFetcherPersistedDestinationKey];
  }
...
+ (NSArray *)activePersistedBackgroundSessions {
  ...
  NSArray *oldBackgroundSessions =
      [userDefaults arrayForKey:kDCGTMSessionFetcherPersistedDestinationKey];
...
+ (NSArray *)fetchersForBackgroundSessions {
  ...
  for (NSString *sessionIdentifier in backgroundSessions) {
    ...
    if (!fetcher) {
      ...
      DCGTMSESSION_ASSERT_DEBUG(fetcher != nil,
                              @"Unexpected invalid session identifier: %@", sessionIdentifier);
      [fetcher beginFetchWithCompletionHandler:nil];
    }
    DCGTM_LOG_BACKGROUND_SESSION(@"%@ restoring session %@ by creating fetcher %@ %p",
...
+ (void)application:(UIApplication *)application
  ...
  DCGTMSessionFetcher *fetcher = [self fetcherWithSessionIdentifier:identifier];
  if (fetcher != nil) {
    fetcher.systemCompletionHandler = completionHandler;
  } else {
    DCGTM_LOG_BACKGROUND_SESSION(@"%@ did not create background session identifier: %@",
...
- (void)setSessionIdentifier:(NSString *)sessionIdentifier {
  DCGTMSESSION_ASSERT_DEBUG(sessionIdentifier != nil, @"Invalid session identifier");
  @synchronized(self) {
    DCGTMSESSION_ASSERT_DEBUG(!_session, @"Unable to set session identifier after session created");
...
- (void)setSessionUserInfo:(NSDictionary *)dictionary {
  @synchronized(self) {
    DCGTMSESSION_ASSERT_DEBUG(_sessionIdentifier == nil, @"Too late to assign userInfo");
...
- (NSDictionary *)sessionIdentifierDefaultMetadata {
  ...
  if (_destinationFileURL) {
    defaultUserInfo[kDCGTMSessionIdentifierDestinationFileURLMetadataKey] =
        [_destinationFileURL absoluteString];
  }
  if (_bodyFileURL) {
    defaultUserInfo[kDCGTMSessionIdentifierBodyFileURLMetadataKey] = [_bodyFileURL absoluteString];
  }
...
- (void)restoreDefaultStateForSessionIdentifierMetadata {
  ...
  NSString *destinationFileURLString = metadata[kDCGTMSessionIdentifierDestinationFileURLMetadataKey];
  if (destinationFileURLString) {
    _destinationFileURL = [NSURL URLWithString:destinationFileURLString];
    DCGTM_LOG_BACKGROUND_SESSION(@"Restoring destination file URL: %@", _destinationFileURL);
  }
  NSString *bodyFileURLString = metadata[kDCGTMSessionIdentifierBodyFileURLMetadataKey];
  if (bodyFileURLString) {
    _bodyFileURL = [NSURL URLWithString:bodyFileURLString];
    DCGTM_LOG_BACKGROUND_SESSION(@"Restoring body file URL: %@", _bodyFileURL);
  }
}
  ...
  if ([metadataScanner scanUpToString:@"_" intoString:NULL] &&
    ...
    NSDictionary *metadataDict =
        [NSJSONSerialization JSONObjectWithData:metadataData
                                        options:0
                                          error:&error];
    DCGTM_LOG_BACKGROUND_SESSION(@"User Info from session identifier: %@ %@",
...
- (void)createSessionIdentifierWithMetadata:(NSDictionary *)metadataToInclude {
  // Session Identifier format: "com.google.<ClassName>_<UUID>_<Metadata in JSON format>
  DCGTMSESSION_ASSERT_DEBUG(!_sessionIdentifier, @"Session identifier already created");
  _sessionIdentifierUUID = [[NSUUID UUID] UUIDString];
  _sessionIdentifier =
      [NSString stringWithFormat:@"%@_%@", kDCGTMSessionIdentifierPrefix, _sessionIdentifierUUID];
  ...
  if ([metadataDict count] > 0) {
    NSData *metadataData = [NSJSONSerialization dataWithJSONObject:metadataDict
                                                           options:0
                                                             error:NULL];
    DCGTMSESSION_ASSERT_DEBUG(metadataData != nil,
...
- (void)failToBeginFetchWithError:(NSError *)error {
  if (error == nil) {
    error = [NSError errorWithDomain:kDCGTMSessionFetcherErrorDomain
                                code:DCGTMSessionFetcherErrorDownloadFailed
                            userInfo:nil];
  }
...
+ (DCGTMSessionCookieStorage *)staticCookieStorage {
  static DCGTMSessionCookieStorage *gCookieStorage = nil;

  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gCookieStorage = [[DCGTMSessionCookieStorage alloc] init];
  });
...
#endif // DCGTM_BACKGROUND_TASK_FETCHING
...
- (void)authorizeRequest {
  ...
  if ([authorizer respondsToSelector:asyncAuthSel]) {
    ...
    DCGTMSESSION_ASSERT_DEBUG(authorizer == nil, @"invalid authorizer for fetch");
...
- (void)authorizer:(id<DCGTMFetcherAuthorizationProtocol>)auth
...
- (void)stopFetchReleasingCallbacks:(BOOL)shouldReleaseCallbacks {
  ...
  id<DCGTMSessionFetcherServiceProtocol> service;
  ...
  // the stack to unwind.
  __autoreleasing DCGTMSessionFetcher *holdSelf = self;
...
- (void)sendStopNotificationIfNeeded {
  ...
  if (sendNow) {
    [[NSNotificationCenter defaultCenter] postNotificationName:kDCGTMSessionFetcherStoppedNotification
                                                        object:self];
  }
}
...
+ (void)setGlobalTestBlock:(DCGTMSessionFetcherTestBlock)block {
#if DCGTM_DISABLE_FETCHER_TEST_BLOCK
  DCGTMSESSION_ASSERT_DEBUG(block == nil, @"test blocks disabled");
#endif
...
willPerformHTTPRedirection:(NSHTTPURLResponse *)redirectResponse
 ...
 completionHandler:(void (^)(NSURLRequest *))handler {
  ...
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ task:%@ willPerformHTTPRedirection:%@ newRequest:%@",
  ...
  @synchronized(self) {
    ...
    if (redirectRequest && redirectResponse) {
      ...
      DCGTMSessionFetcherWillRedirectBlock willRedirectBlock = _willRedirectBlock;
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))handler {
  ...
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ dataTask:%@ didReceiveResponse:%@",
  ...
  void (^accumulateAndFinish)(NSURLSessionResponseDisposition) =
      ...
      @synchronized(self) {
        ...
        if (hadPreviousData && (dispositionValue != NSURLSessionResponseCancel)) {
          ...
          DCGTMSessionFetcherAccumulateDataBlock accumulateBlock = _accumulateDataBlock;
          if (accumulateBlock) {
            [self invokeOnCallbackQueueUnlessStopped:^{
                accumulateBlock(nil);
            }];
  ...
  DCGTMSessionFetcherDidReceiveResponseBlock receivedResponseBlock;
  @synchronized(self) {
...
didBecomeDownloadTask:(NSURLSessionDownloadTask *)downloadTask {
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ dataTask:%@ didBecomeDownloadTask:%@",
...
didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge
 completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition,
  ...
  [self setSessionTask:task];
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ task:%@ didReceiveChallenge:%@",
        ...
        if (serverTrust == NULL) {
          ...
          void (^callback)(SecTrustRef, BOOL) = ^(SecTrustRef trustRef, BOOL allow){
            if (allow) {
              ...
              DCGTMSESSION_LOG_DEBUG(@"Cancelling authentication challenge for %@", [_request URL]);
          ...
          if (_allowInvalidServerCertificates) {
            ...
            dispatch_async(evaluateBackgroundQueue, ^{
              ...
              BOOL shouldAllow;
              OSStatus trustError;
              @synchronized([DCGTMSessionFetcher class]) {
                trustError = SecTrustEvaluate(serverTrust, &trustEval);
              }
              if (trustError != errSecSuccess) {
                DCGTMSESSION_LOG_DEBUG(@"Error %d evaluating trust for %@",
                ...
                if (trustEval == kSecTrustResultUnspecified
                  ...
                  shouldAllow = NO;
                  DCGTMSESSION_LOG_DEBUG(@"Challenge SecTrustResultType %u for %@, properties: %@",
  ...
  if (handler) {
    [self invokeOnCallbackQueueUnlessStopped:^{
        ...
        dispatch_group_async(_callbackGroup, dispatch_get_main_queue(), ^{
            ...
            if (data) {
              userInfo[kDCGTMSessionFetcherCompletionDataKey] = data;
            }
            if (error) {
              userInfo[kDCGTMSessionFetcherCompletionErrorKey] = error;
            }
            NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
            [nc postNotificationName:kDCGTMSessionFetcherCompletionInvokedNotification
                              object:self
                            userInfo:userInfo];
        });
    }];
...
- (void)URLSession:(NSURLSession *)session
 ...
 needNewBodyStream:(void (^)(NSInputStream *bodyStream))completionHandler {
  ...
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ task:%@ needNewBodyStream:",
                           [self class], self, session, uploadTask);
  @synchronized(self) {
    DCGTMSessionFetcherBodyStreamProvider provider = _bodyStreamProvider;
#if !STRIP_DCGTM_FETCH_LOGGING
    ...
    if (provider) {
      ...
      DCGTMSESSION_ASSERT_DEBUG(NO, @"NSURLSession expects a stream provider");
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend {
  [self setSessionTask:task];
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ task:%@ didSendBodyData:%lld"
  ...
  [self invokeOnCallbackQueueUnlessStopped:^{
      DCGTMSessionFetcherSendProgressBlock progressBlock;
      @synchronized(self) {
...
- (void)URLSession:(NSURLSession *)session
  ...
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ dataTask:%@ didReceiveData:%p (%llu bytes)",
  ...
  @synchronized(self) {
    ...
    if (accumulateBlock) {
      ...
      if (_receivedProgressBlock) {
        [self invokeOnCallbackQueueUnlessStopped:^{
            DCGTMSessionFetcherReceivedProgressBlock progressBlock;
            @synchronized(self) {
...
- (void)URLSession:(NSURLSession *)session
 ...
 completionHandler:(void (^)(NSCachedURLResponse *cachedResponse))completionHandler {
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ dataTask:%@ willCacheResponse:%@ %@",
                           [self class], self, session, dataTask,
                           proposedResponse, proposedResponse.response);
  DCGTMSessionFetcherWillCacheURLResponseBlock callback;
  @synchronized(self) {
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite {
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ downloadTask:%@ didWriteData:%lld"
  ...
  [self invokeOnCallbackQueueUnlessStopped:^{
      DCGTMSessionFetcherDownloadProgressBlock progressBlock;
      @synchronized(self) {
...
expectedTotalBytes:(int64_t)expectedTotalBytes {
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ downloadTask:%@ didResumeAtOffset:%lld"
...
didFinishDownloadingToURL:(NSURL *)downloadLocationURL {
  ...
  [self setSessionTask:downloadTask];
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ downloadTask:%@ didFinishDownloadingToURL:%@",
  ...
  @synchronized(self) {
    ...
    DCGTM_LOG_BACKGROUND_SESSION(@"%@ %p Moved download from \"%@\" to \"%@\"  %@",
                               [self class], self,
...
didCompleteWithError:(NSError *)error {
  [self setSessionTask:task];
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ task:%@ didCompleteWithError:%@",
...
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session {
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSessionDidFinishEventsForBackgroundURLSession:%@",
                           [self class], self, session);
  [self removePersistedBackgroundSessionFromDefaults];

  DCGTMSessionFetcherSystemCompletionHandler handler;
  @synchronized(self) {
    handler = self.systemCompletionHandler;
    self.systemCompletionHandler = nil;
  }
  if (handler) {
    DCGTM_LOG_BACKGROUND_SESSION(@"%@ %p Calling system completionHandler", [self class], self);
    handler();
...
- (void)URLSession:(NSURLSession *)session didBecomeInvalidWithError:(NSError *)error {
  ...
  DCGTM_LOG_SESSION_DELEGATE(@"%@ %p URLSession:%@ didBecomeInvalidWithError:%@",
      ...
      if (shouldRetry) {
        ...
        if (error == nil) {
          ...
          if ([_downloadedData length] > 0) {
            userInfo = @{ kDCGTMSessionFetcherStatusDataKey : _downloadedData };
          }
          error = [NSError errorWithDomain:kDCGTMSessionFetcherStatusDomain
                                      code:status
                                  userInfo:userInfo];
        } else {
...
- (BOOL)isRetryError:(NSError *)error {
  ...
  struct RetryRecord retries[] = {
    { kDCGTMSessionFetcherStatusDomain, 408 }, // request timeout
    { kDCGTMSessionFetcherStatusDomain, 502 }, // failure gatewaying to another server
    { kDCGTMSessionFetcherStatusDomain, 503 }, // service unavailable
    { kDCGTMSessionFetcherStatusDomain, 504 }, // request timeout
...
- (void)shouldRetryNowForStatus:(NSInteger)status
                          error:(NSError *)error
                       response:(DCGTMSessionFetcherRetryResponse)response {
  ...
  @synchronized(self) {
    ...
    BOOL isFirstAuthError = (_authorizer != nil
                             ...
                             && status == DCGTMSessionFetcherStatusUnauthorized); // 401
    ...
    if (canRetry) {
      ...
      NSError *statusError = [NSError errorWithDomain:kDCGTMSessionFetcherStatusDomain
                                                 code:status
                                             userInfo:userInfo];
      if (error == nil) {
        error = statusError;
      ...
      DCGTMSessionFetcherRetryBlock retryBlock = _retryBlock;
      if (retryBlock) {
        [self invokeOnCallbackQueueUnlessStopped:^{
...
- (void)beginRetryTimer {
  ...
  NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
  [nc postNotificationName:kDCGTMSessionFetcherRetryDelayStartedNotification
                    object:self];
}
...
- (void)destroyRetryTimer {
  ...
  if (shouldNotify) {
    NSNotificationCenter *defaultNC = [NSNotificationCenter defaultCenter];
    [defaultNC postNotificationName:kDCGTMSessionFetcherRetryDelayStoppedNotification
                             object:self];
  }
}
...
- (DCGTMSessionFetcherSystemCompletionHandler)systemCompletionHandler {
  return [[self class] systemCompletionHandlerForSessionIdentifier:_sessionIdentifier];
}

- (void)setSystemCompletionHandler:(DCGTMSessionFetcherSystemCompletionHandler)systemCompletionHandler {
...
+ (DCGTMSessionFetcherSystemCompletionHandler)systemCompletionHandlerForSessionIdentifier:(NSString *)sessionIdentifier {
...
- (void)setDestinationFileURL:(NSURL *)destinationFileURL {
  @synchronized(self) {
    ...
    if (_sessionIdentifier) {
      ...
      DCGTMSESSION_ASSERT_DEBUG([oldFilename isEqualToString:newFilename],
...
@implementation DCGTMSessionFetcher (BackwardsCompatibilityOnly)
...
- (void)setCookieStorageMethod:(NSInteger)method {
  ...
  //
  // Clients using the DCGTMSessionFetcher class should set the cookie storage explicitly
  // themselves.
  NSHTTPCookieStorage *storage = nil;
  switch(method) {
    case 0:  // kDCGTMHTTPFetcherCookieStorageMethodStatic
             // nil storage will use [[self class] staticCookieStorage] when the fetch begins.
      break;
    case 1:  // kDCGTMHTTPFetcherCookieStorageMethodFetchHistory
             // Do nothing; use whatever was set by the fetcher service.
      return;
    case 2:  // kDCGTMHTTPFetcherCookieStorageMethodSystemDefault
      storage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
      break;
    case 3:  // kDCGTMHTTPFetcherCookieStorageMethodNone
             // Create temporary storage for this fetcher only.
      storage = [[DCGTMSessionCookieStorage alloc] init];
      break;
    default:
      DCGTMSESSION_ASSERT_DEBUG(0, @"Invalid cookie storage method: %d", (int)method);
  }
...
@implementation DCGTMSessionCookieStorage {
...
- (void)internalSetCookie:(NSHTTPCookie *)newCookie {
  ...
  DCGTMSESSION_ASSERT_DEBUG(isValidCookie, @"invalid cookie: %@", newCookie);
...
- (void)getCookiesForTask:(NSURLSessionTask *)task
        completionHandler:(void (^)(DCGTM_NSArrayOf(NSHTTPCookie *) *))completionHandler {
...
- (NSHTTPCookie *)cookieMatchingCookie:(NSHTTPCookie *)cookie {
  ...
  DCGTMSESSION_ASSERT_DEBUG(name && domain && path,
...
#pragma unused(procInfo)
    ...
    NSString *model = DCGTMFetcherCleanedUserAgentString(rawModel);
    ...
    if (uname(&unameRecord) == 0) {
      NSString *machineName = [NSString stringWithCString:unameRecord.machine
                                                 encoding:NSUTF8StringEncoding];
      hardwareModel = DCGTMFetcherCleanedUserAgentString(machineName);
    } else {
      hardwareModel = @"unk";
    }
#endif
...
NSString *DCGTMFetcherStandardUserAgentString(NSBundle *bundle) {
  NSString *result = [NSString stringWithFormat:@"%@ %@",
                      DCGTMFetcherApplicationIdentifier(bundle),
                      DCGTMFetcherSystemVersionString()];
  return result;
}

NSString *DCGTMFetcherApplicationIdentifier(NSBundle *bundle) {
  @synchronized([DCGTMSessionFetcher class]) {
    ...
    identifier = [bundle objectForInfoDictionaryKey:@"DCGTMUserAgentID"];
    ...
    identifier = DCGTMFetcherCleanedUserAgentString(identifier);
    ...
    // Clean up whitespace and special characters
    version = DCGTMFetcherCleanedUserAgentString(version);
