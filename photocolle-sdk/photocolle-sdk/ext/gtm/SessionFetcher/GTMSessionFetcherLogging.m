...
@interface DCGTMSessionFetcher (DCGTMHTTPFetcherLoggingUtilities)
...
@implementation DCGTMSessionFetcher (DCGTMSessionFetcherLogging)
...
#if !TARGET_OS_IPHONE && !DCGTM_SKIP_LOG_XMLFORMAT
...
- (void)logFetchWithError:(NSError *)error {
  ...
  if (gIsLoggingToFile) {
    ...
    if (!gReportedLoggingPath) {
      ...
      NSLog(@"DCGTMSessionFetcher logging to \"%@\"", parentDir);
    }
#endif
  }
}
...
- (NSInputStream *)loggedInputStreamForInputStream:(NSInputStream *)inputStream {
  ...
  if (![DCGTMSessionFetcher isLoggingEnabled]) return inputStream;
  ...
  DCGTMReadMonitorInputStream *readMonitorInputStream = (DCGTMReadMonitorInputStream *)inputStream;
...
- (DCGTMSessionFetcherBodyStreamProvider)loggedStreamProviderForStreamProvider:
  ...
  if (![DCGTMSessionFetcher isLoggingEnabled]) return streamProvider;
  ...
  DCGTMSessionFetcherBodyStreamProvider loggedStreamProvider =
      ^(DCGTMSessionFetcherBodyStreamProviderResponse response) {
...
@implementation DCGTMSessionFetcher (DCGTMSessionFetcherLoggingUtilities)
