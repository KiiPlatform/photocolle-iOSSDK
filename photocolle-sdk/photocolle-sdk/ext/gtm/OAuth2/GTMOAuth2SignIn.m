...
@implementation DCGTMOAuth2SignIn
...
- (BOOL)startWebRequest {
  ...
  NSString *paramStr = [DCGTMOAuth2Authentication encodedQueryParametersForDictionary:paramsDict];
...
- (BOOL)titleChanged:(NSString *)title {
  ...
  if (spaceIndex != NSNotFound) {
    ...
    NSDictionary *dict = [DCGTMOAuth2Authentication dictionaryWithResponseString:responseStr];
...
- (void)authCodeObtained {
  ...
  if ([code length] > 0) {
    ...
    [nc postNotificationName:kDCGTMOAuth2UserSignedIn
                      object:self
                    userInfo:nil];
  } else {
    ...
    if ([errStr length] > 0) {
      userInfo = [NSDictionary dictionaryWithObject:errStr
                                             forKey:kDCGTMOAuth2ErrorMessageKey];
    }
...
finishedWithFetcher:(DCGTMHTTPFetcher *)fetcher
  ...
  if (error == nil
      ...
      && [self.authentication.serviceProvider isEqual:kDCGTMOAuth2ServiceProviderGoogle]) {
...
static void ReachabilityCallBack(SCNetworkReachabilityRef target,
  ...
  DCGTMOAuth2SignIn *signIn = (DCGTMOAuth2SignIn *)info;
...
- (void)reachabilityTarget:(SCNetworkReachabilityRef)reachabilityRef
  ...
  if (isConnected) {
    ...
    if (hasNotifiedNetworkLoss_) {
      ...
      NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
      [nc postNotificationName:kDCGTMOAuth2NetworkFound
                        object:self
                      userInfo:nil];
...
- (void)reachabilityTimerFired:(NSTimer *)timer {
  ...
  if (!hasNotifiedNetworkLoss_) {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:kDCGTMOAuth2NetworkLost
                      object:self
                    userInfo:nil];
...
+ (void)revokeTokenForGoogleAuthentication:(DCGTMOAuth2Authentication *)auth {
  if (auth.refreshToken != nil
      && auth.canAuthorize
      && [auth.serviceProvider isEqual:kDCGTMOAuth2ServiceProviderGoogle]) {
    ...
    NSString *encoded = [DCGTMOAuth2Authentication encodedOAuthValueForString:token];
    if (encoded != nil) {
