...
NSString *const kDCGTMOAuth2KeychainErrorDomain = @"com.google.DCGTMOAuthKeychain";

NSString *const kDCGTMOAuth2CookiesWillSwapOut = @"kDCGTMOAuth2CookiesWillSwapOut";
NSString *const kDCGTMOAuth2CookiesDidSwapIn   = @"kDCGTMOAuth2CookiesDidSwapIn";

static NSString * const kDCGTMOAuth2AccountName = @"OAuth";
static DCGTMOAuth2Keychain* gDCGTMOAuth2DefaultKeychain = nil;

@interface DCGTMOAuth2ViewControllerTouch()
@property (nonatomic, copy) NSURLRequest *request;
@property (nonatomic, copy) NSArray *systemCookies;
@property (nonatomic, copy) NSArray *signInCookies;
@end

@implementation DCGTMOAuth2ViewControllerTouch
...
+ (id)controllerWithScope:(NSString *)scope
             ...
             clientSecret:(NSString *)clientSecret
         keychainItemName:(NSString *)keychainItemName
        completionHandler:(DCGTMOAuth2ViewControllerCompletionHandler)handler {
  return [[[self alloc] initWithScope:scope
                             clientID:clientID
                         clientSecret:clientSecret
                     keychainItemName:keychainItemName
...
#endif // !DCGTM_OAUTH2_SKIP_GOOGLE_SUPPORT

+ (id)controllerWithAuthentication:(DCGTMOAuth2Authentication *)auth
...
#if !DCGTM_OAUTH2_SKIP_GOOGLE_SUPPORT
+ (DCGTMOAuth2Authentication *)authForGoogleFromKeychainForName:(NSString *)keychainItemName
                                                     clientID:(NSString *)clientID
                                                 clientSecret:(NSString *)clientSecret {
...
+ (DCGTMOAuth2Authentication *)authForGoogleFromKeychainForName:(NSString *)keychainItemName
  ...
  DCGTMOAuth2Authentication *auth;
  auth = [DCGTMOAuth2Authentication authenticationWithServiceProvider:kDCGTMOAuth2ServiceProviderGoogle
                                                           tokenURL:tokenURL
...
+ (BOOL)authorizeFromKeychainForName:(NSString *)keychainItemName
                      authentication:(DCGTMOAuth2Authentication *)newAuth
                               error:(NSError **)error {
  ...
  NSString *password = [keychain passwordForService:keychainItemName
                                            account:kDCGTMOAuth2AccountName
                                              error:error];
  if (password != nil) {
...
+ (BOOL)saveParamsToKeychainForName:(NSString *)keychainItemName
                     authentication:(DCGTMOAuth2Authentication *)auth {
  ...
  if (![auth canAuthorize]) {
    if (error) {
      *error = [NSError errorWithDomain:kDCGTMOAuth2ErrorDomain
                                   code:DCGTMOAuth2ErrorTokenUnavailable
                               userInfo:nil];
    }
    return NO;
  }
...
- (void)notifyWithName:(NSString *)name
               webView:(UIWebView *)webView
                  kind:(NSString *)kind {
  BOOL isStarting = [name isEqual:kDCGTMOAuth2WebViewStartedLoading];
  ...
  NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
                        webView, kDCGTMOAuth2WebViewKey,
                        kind, kDCGTMOAuth2WebViewStopKindKey, // kind may be nil
                        nil];
...
+ (Class)signInClass {
  if (gSignInClass == Nil) {
    gSignInClass = [DCGTMOAuth2SignIn class];
  }
  return gSignInClass;
}
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:kDCGTMOAuth2CookiesWillSwapOut
                                                      object:self
                                                    userInfo:nil];
  [self swapOutCookies];
...
- (void)viewWillDisappear:(BOOL)animated {
  if (![self isBeingObscured:self]) {
    if (!didDismissSelf_) {
      ...
      [self notifyWithName:kDCGTMOAuth2WebViewStoppedLoading
                   webView:self.webView
                      kind:kDCGTMOAuth2WebViewCancelled];
  ...
  [self swapInCookies];
  [[NSNotificationCenter defaultCenter] postNotificationName:kDCGTMOAuth2CookiesDidSwapIn
                                                      object:self
                                                    userInfo:nil];
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
  [self notifyWithName:kDCGTMOAuth2WebViewStartedLoading
               webView:webView
                  kind:nil];
  [self updateUI];
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
  [self notifyWithName:kDCGTMOAuth2WebViewStoppedLoading
               webView:webView
                  kind:kDCGTMOAuth2WebViewFinished];
  ...
  if ([title length] > 0) {
    ...
    NSAssert([result integerValue] == 2, @"DCGTMOAuth2: Javascript is required");
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
  [self notifyWithName:kDCGTMOAuth2WebViewStoppedLoading
               webView:webView
                  kind:kDCGTMOAuth2WebViewFailed];
...
@implementation DCGTMOAuth2Keychain

+ (DCGTMOAuth2Keychain *)defaultKeychain {
  if (gDCGTMOAuth2DefaultKeychain == nil) {
    gDCGTMOAuth2DefaultKeychain = [[self alloc] init];
  }
  return gDCGTMOAuth2DefaultKeychain;
}


// For unit tests: allow setting a mock object
+ (void)setDefaultKeychain:(DCGTMOAuth2Keychain *)keychain {
  if (gDCGTMOAuth2DefaultKeychain != keychain) {
    [gDCGTMOAuth2DefaultKeychain release];
    gDCGTMOAuth2DefaultKeychain = [keychain retain];
  }
}

- (NSString *)keyForService:(NSString *)service account:(NSString *)account {
  return [NSString stringWithFormat:@"com.google.DCGTMOAuth.%@%@", service, account];
}
...
- (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
  ...
  if (0 < [service length] && 0 < [account length]) {
    ...
    if (result == nil && error != NULL) {
      *error = [NSError errorWithDomain:kDCGTMOAuth2KeychainErrorDomain
                                   code:kDCGTMOAuth2KeychainErrorNoPassword
                               userInfo:nil];
    }
  } else if (error != NULL) {
    *error = [NSError errorWithDomain:kDCGTMOAuth2KeychainErrorDomain
                                 code:kDCGTMOAuth2KeychainErrorBadArguments
                             userInfo:nil];
  }
  return result;
...
// iPhone
- (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
  OSStatus status = DCGTMOAuth2KeychainErrorBadArguments;
  NSString *result = nil;
  if (0 < [service length] && 0 < [account length]) {
